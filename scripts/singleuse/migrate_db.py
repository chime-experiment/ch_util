...
def conn_list(lay_id, db):
    ...
    cur = db.cursor()
    cur.execute(
        "SELECT comp_sn1, comp_sn2 FROM layout_conn WHERE "
...
def prop_list(lay_id, db):
    ...
    cur = db.cursor()
    cur.execute(
        "SELECT comp_sn, name, value FROM layout_prop JOIN prop_type ON "
...
if True:
    ...
    for row in cur.fetchall():
        ...
        try:
            ...
            if row[1]:
                layout.connexion.from_pair(row[0], row[1]).make(
                    time=row[5], permanent=True
                )

# Add permanent properties!
if True:
    cur.execute(
        "SELECT p.*, t.name FROM layout_prop p "
    ...
    for row in cur.fetchall():
        ...
        comp.set_property(
            layout.property_type.get(name=row[7]),
            row[5],
            time=e.start.time,
            notes=row[6],
        )
...
if True:
    ...
    last_time = None
    cur_lay.execute(
        "SELECT l.id, l.user_id, change_start, change_end, l.name, "
        "l.notes FROM "
    ...
    for lay_row in cur_lay.fetchall():
        ...
        if last_time:
            layout.add_layout_tag(
                last_name, start_time=last_time, end_time=lay_row[2], notes=last_notes
            )
        last_time = lay_row[3]
        last_name = lay_row[4]
