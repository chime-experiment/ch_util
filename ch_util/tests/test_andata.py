...
class TestReader(unittest.TestCase):
    ...
    def test_select_read(self):
        ...
        self.assertEqual(set(data.datasets.keys()),
            {'vis', 'timestamp_cpu_s', 'timestamp_cpu_us',
            'timestamp_fpga_count', 'gain'})
    ...
    def test_select_freq_range(self):
        ...
        self.assertTrue(np.all(abs(diff_centres) <= 400./1024))
    ...
    def test_select_frequencies(self):
        ...
        self.assertTrue(np.all(abs(diff_centres) <= 400./1024/2))
        self.assertRaises(ValueError, self.reader.select_freq_physical, [324.])
...
class TestLoadACQ(unittest.TestCase):
    ...
    def test_subset_vis(self):
        data = andata.AnData.from_acq_h5(acq_fname, prod_sel=[1, 3, 9],
                                         freq_sel=6)
...
class TestAnData(unittest.TestCase):
    ...
    def test_vis_shortcuts(self):
        ...
        self.assertTrue(np.allclose(self.data.vis[0:2:,0,1:3:9],
                        vis[0:2:,0,1:3:9]))
...
class TestConcatenate(unittest.TestCase):
    ...
    def setUp(self):
        data_list = []
        data_list = [andata.AnData.from_acq_h5(fname) for fname in
                     acq_fname_list]
    ...
    def test_works(self):
        ...
        self.assertEqual(len(merged_data.index_map['time']),
                         len(self.right_answer.index_map['time']))
...
def check_result(self, data, ntime):
    ...
    for dset_name in data.datasets.keys():
        if dset_name[:10] == "serial_adc":
            ...
            self.assertEqual(data.datasets[dset_name].attrs['cal'],
                             ATEL)
