...
class TestLoadACQ(unittest.TestCase):
    ...
    def test_load_fname(self):
        data = andata.AnData.from_acq_h5(
            archive_acq_fname_list_2_0[0],
            start=3,
            stop=10,
            prod_sel=[1, 3, 7],
            freq_sel=slice(8, 18),
    ...
    def test_multi_file(self):
        ...
        psel = [0, 16, 31]
        data = andata.AnData.from_acq_h5(
            archive_acq_fname_list_2_0, start=3, stop=43, prod_sel=psel, freq_sel=fsel
        )
    ...
    def test_no_datasets(self):
        datasets = ()
        data = andata.AnData.from_acq_h5(
            archive_acq_fname_list_2_0, start=3, stop=43, datasets=datasets
        )
...
class TestRemapInputs(unittest.TestCase):
    ...
    def test_load_prod_sel(self):
        data = andata.CorrData.from_acq_h5(archive_acq_fname_list_2_0, prod_sel=[0, 1])
        self.assertTrue(np.all(data.index_map["prod"] == self.prod[[0, 1]]))
        self.assertTrue(np.all(data.index_map["input"] == self.remapped_input[[0, 1]]))

    def test_load_prod_sel2(self):
        data = andata.CorrData.from_acq_h5(archive_acq_fname_list_2_0, prod_sel=[4])
        self.assertTrue(np.all(data.index_map["prod"] == self.prod[1]))
        self.assertTrue(np.all(data.index_map["input"] == self.remapped_input[[0, 4]]))

    def test_load_input_sel(self):
        data = andata.CorrData.from_acq_h5(archive_acq_fname_list_2_0, input_sel=0)
        self.assertTrue(np.all(data.index_map["prod"] == self.prod[[0]]))
        self.assertTrue(np.all(data.index_map["input"] == self.remapped_input[[0]]))

    def test_load_input_sel2(self):
        data = andata.CorrData.from_acq_h5(archive_acq_fname_list_2_0, input_sel=[4, 5])
        self.assertTrue(np.all(data.index_map["prod"] == self.prod[[0, 1, 16]]))
        self.assertTrue(np.all(data.index_map["input"] == self.remapped_input[[4, 5]]))

    def test_load_input_sel3(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_0, input_sel=np.s_[4:10]
        )
        self.assertEqual(len(data.index_map["prod"]), 6 * 7 // 2)
        self.assertTrue(
            np.all(data.index_map["input"] == self.remapped_input[np.s_[4:10]])
        )
...
class TestApplyGains2_0(unittest.TestCase):
    ...
    def test_input_sel(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_0, input_sel=np.s_[4:8], datasets=("vis",)
        )
        ...
        prod_sel, prod_map, input_sel, input_map = _resolve_prod_input_sel(
            None,
            self.all_data.index_map["prod"],
            np.s_[4:8],
            self.all_data.index_map["input"],
        )
        self.assertTrue(np.all(data.vis[:] == self.all_data.vis[:, prod_sel, :]))

    def test_prod_sel(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_0, prod_sel=np.s_[5:14], datasets=("vis",)
        )
        ...
        self.assertTrue(
            np.all(
                data.datasets["gain"][:] == self.all_data.datasets["gain"][:, input_sel]
            )
        )
        # Make sure they where properly applied.
        self.assertTrue(np.all(data.vis[:] == self.all_data.vis[:, prod_sel, :]))
...
class TestApplyGains2_2(unittest.TestCase):
    ...
    def test_input_sel(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_2, input_sel=np.s_[4:8], datasets=("vis",)
        )
        self.assertEqual(data.datasets["gain"].shape[1], 4)
        # Make sure the gains were properly calculated.
        self.assertTrue(
    ...
    def test_prod_sel(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_2, prod_sel=np.s_[5:14]
        )
        self.assertEqual(data.datasets["gain"].shape[1], 10)
        prod_sel, prod_map, input_sel, input_map = _resolve_prod_input_sel(
            np.s_[5:14],
    ...
    def test_prod_sel_int_auto(self):
        data = andata.CorrData.from_acq_h5(archive_acq_fname_list_2_2, prod_sel=0)
        self.assertEqual(data.datasets["gain"].shape[1], 1)
        prod_sel, prod_map, input_sel, input_map = _resolve_prod_input_sel(
            0, self.all_data.index_map["prod"], None, self.all_data.index_map["input"]
        )
        # Make sure the gains were properly calculated.
        self.assertTrue(
            np.all(
    ...
    def test_prod_sel_fancy(self):
        data = andata.CorrData.from_acq_h5(
            archive_acq_fname_list_2_2,
            prod_sel=np.arange(8000),
            datasets=("vis",),
            freq_sel=[0, 1],
        )
