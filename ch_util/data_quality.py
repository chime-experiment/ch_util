...
def good_channels(data, gain_tol=10.0, noise_tol=2.0, fit_tol=0.02,
                  test_freq=0, noise_synced=None, inputs=None,
                  res_plot=False, verbose=True):
    ...
    if 'gpu.gpu_intergration_period' in data.attrs:
        ...
        t_step = (data.index_map['time']['ctime'][1]-
                  data.index_map['time']['ctime'][0]) # Integration time
    ...
    if noise_synced == False:
        ...
        if is_gated_format:
            ...
            msg = ('Warning: noise_synced=True argument given '+
    ...
    autos_index, autos_chan = _get_autos_index(
                                prod_array_full)
    ...
    if 'gain' in data:
        ...
        if verbose:
            msg = ('Could not obtain digital gains information from data. '+
                   'Ignoring gains test.')
            print(msg)
    ...
    if np.sum(stand_chans)>50 and len(cut_tmstp)>100: 
        ...
        good_fit, popt, perr, sky = _fit_template(Ts,cut_vis,
                                                  tol=fit_tol)

        # Create plot with results, if res_plot is True:
        if res_plot:
            print('Generating plots')
            _create_plot(visi,tmstp,cut_tmstp,sky,popt,test_chans,
                         good_gains,good_noise,good_fit)
    else:
        if verbose:
    ...
    if verbose:
        ...
        Nact, Nnoisy, Ngains, Nfit, Nbad = _stats_print(good_noise,
                                    good_gains,good_fit,test_chans)
...
def _radiom_noise(trace,n_samp,wind=100):
    ...
    t_w = [ trace[ii*wind:(ii+1)*wind]
            for ii in range(int(len(trace)//wind)) ]
    ...
    t_s = [ [ t_w[ii][jj]-t_w[ii][jj+1]
        for jj in range(0,int(wind),2) ]
    ...
    rms = [ np.median([ np.abs(entry[ii]-np.median(entry))
                for ii in range(len(entry)) ])*1.4826/np.sqrt(2)
                for entry in t_s ]
...
def _cut_daytime(visi,tmstp):
    ...
    if len(risings)==0 and len(settings)==0:
        ...
        cut_low = max(0,idx-int(20.*60./tstp)) # lower limit of time cut
        ...
        # Determine time limits to cut:
        # (20 min after setting and before rising, if within range)
        cut_up = min(len(tmstp),idx+int(20.*60./tstp)) # upper limit of time to cut
        ...
        for cp in cut_pairs:
            ...
            # Determine time limits to cut:
            # (20 min after setting and before rising, if within range)
            cut_low = max(0,idx_low-int(20.*60./tstp)) # lower limit of time cut
            cut_up = min(len(tmstp_remain),idx_up+int(20.*60./tstp)) # upper limit of time to cut

            if len(tmstp_remain[:cut_low])>0:
                cut_vis.append(vis_remain[:,:cut_low])
                cut_tmstp.append(tmstp_remain[:cut_low]) # Append times before rising to cut_tmstp
            vis_remain = vis_remain[:,cut_up:]
            tmstp_remain = tmstp_remain[cut_up:] # Use times after setting for further cuts
...
def _gains_test(data,test_freq,test_chans,tol):
    ...
    gains = abs(np.array([ data.gain[test_freq,input_map.index(chan),0]
...
def _stats_print(good_noise,good_gains,good_fit,test_chans):
    ...
    if good_noise is not None:
        ...
        print ('Noisy channels: {0} out of {1} active channels ({2:2.1f}%)'.format(Nnoisy,Nact,Nnoisy*100/Nact))
        good_chans = good_chans*good_noise
    else:
        Nnoisy = None
    if good_gains is not None:
        Ngains = Nact - int(np.sum(good_gains))
        print('High digital gains: {0} out of {1} active channels ({2:2.1f}%)'.format(Ngains,Nact,Ngains*100/Nact))
        good_chans = good_chans*good_gains
    else:
        Ngains = None
    if good_fit is not None:
        Nfit = Nact - int(np.sum(good_fit))
        print('Bad fit to T_sky: {0} out of {1} active channels ({2:2.1f}%)'.format(Nfit,Nact,Nfit*100/Nact))
    ...
    if not ((good_noise is None) and (good_gains is None) 
                                  and (good_fit is None)):
        Nbad = Nact - int(np.sum(good_chans))
        print('Overall bad: {0} out of {1} active channels ({2:2.1f}%)\n'.format(Nbad,Nact,Nbad*100/Nact))
    else:
        Nbad = None
...
def _cut_sun_transit(cut_vis,tmstp,tcut=120.):
    ...
    for st in sun_trans:
        ...
        cut_low = max(0,idx-int(tcut*60./tstp))
        # upper limit of time cut:
        cut_up = min(len(cut_tmstp),idx+int(tcut*60./tstp))

        # Cut times of solar transit:
        cut_vis = np.concatenate( (cut_vis[:,:cut_low],
                                cut_vis[:,cut_up:]),axis=1 )
        cut_tmstp = np.concatenate( (cut_tmstp[:cut_low],
                                        cut_tmstp[cut_up:]) )
...
def _median_filter(visi,ks=3):
    ...
    cut_vis = np.array([ medfilt(visi[jj,:].real,kernel_size=ks) 
                               for jj in range(visi.shape[0]) ])
    return cut_vis
...
def _get_template(cut_vis_full,stand_chans):
    ...
    cut_vis = np.array([ cut_vis[jj,:]
            for jj in range(cut_vis.shape[0])
            if stand_chans[jj] ])
    ...
    vis_test = np.array([ cut_vis[jj,:]-np.min(cut_vis[jj,:])
                                    for jj in range(Nchans) ])
    # Normalize visibilities:
    vis_test = np.array([ vis_test[jj,:]/
                (np.max(vis_test[jj,:])-np.min(vis_test[jj,:]))
                        for jj in range(vis_test.shape[0]) ])
    medn = np.median(vis_test,axis=0) # Median visibility across channels
    devs = [ np.sum(abs(entry-medn))
            for entry in vis_test ] # Deviations sumed in time
    ...
    cut_vis = np.array([ cut_vis[jj,:]
            for jj in range(len(cut_vis))
                if not (jj in del_ind) ])
    ...
    while ss_diff > 0.01 or ss < 3. :
        ...
        tries += 1
        if tries == 100:
            msg = ('SVD search for Tsky at freq {0} did NOT converge.\n'+
                            'Bad channels list might not be accurate.')
            print(msg)
            break

    # Solution could have negative gains and negative T sky:
    if np.sum(gn) < 0.: # Most gains are < 0 means sign is wrong
...
def _fit_template(Ts,cut_vis,tol):
    ...
    for ii in range(Nchans):
        neg_gain = popt[ii][0] < 0. # Negative gains fail the test
...
def _create_plot(visi,tmstp,cut_tmstp,sky,popt,test_chans,
                 good_gains,good_noise,good_fit):
    ...
    title = 'Good channels result for {0}'.format(ch_eph.unix_to_datetime(tmstp1[0]).date())
    ...
    if (tmstp1[-1]-tmstp1[0])/(24.*3600.) > 3.:
    ...
    for ii in range(len(visi1)):
        ...
        tm_brd = (time_pl1[-1]-time_pl1[0])/10.
        ...
        plt.ylim(med-7.*mad,med+7.*mad)

        # labels:
        plt.ylabel('Ch{0} (Sl.{1})'.format(chan,get_slot(chan)),
                                                        fontsize=8)

        # Hide numbering:
        frame = plt.gca()
        frame.axes.get_yaxis().set_ticks([])
        if ( (chan!=63) and (chan!=127) and
            (chan!=191) and (chan!=255) ):
            ...
            frame.tick_params(axis='both', which='major',
                                                labelsize=10)
            frame.tick_params(axis='both', which='minor',
                                                 labelsize=8)
            if chan==127:
                # Put x-labels on bottom plots:
                if time_unit == 'days':
                    plt.xlabel('Time (days since {0} UTC)'.format(
                                ch_eph.unix_to_datetime(tmstp1[0])),
                                                        fontsize=10)
                else:
                    plt.xlabel('Time (hours since {0} UTC)'.format(
                                ch_eph.unix_to_datetime(tmstp1[0])),
                                                        fontsize=10)
