...
CR = 2.*np.pi/180. # Cylinder rotation in radians
R = np.array([ [np.cos(CR),-np.sin(CR)],
            [np.sin(CR),np.cos(CR)] ]) # Cylinder rotation matrix
C = 2.9979E8
PHI = ephemeris.CHIMELATITUDE*np.pi/180. # DRAO Latitue
SD = 24.*3600.*ephemeris.SIDEREAL_S # Sidereal day
...
class FeedLocator(object):
    ...
    def __init__(self,vis1,vis2,tm1,tm2,src1,src2,freqs,prods,inputs,pstns0,bsipts=None):
        ...
        self.tt1 = ephemeris.transit_times(self.source1,
                        self.tm1[0],self.tm1[-1])[0]+TTCORR[self.source1.name]
        if self.source2 is not None:
            self.tt2 = ephemeris.transit_times(self.source2,
                        self.tm2[0],self.tm2[-1])[0]+TTCORR[self.source2.name]
    ...
    def get_c_ydist(self,ph1=None,ph2=None,good_freqs=None,tol=1.5,Nmax=20):#knl=3,dfacts=[5,11,17],rms_tol=1.):#,stptol=1.5,stpdev=1.):
        ...
        def yparams(xx,yy):
            ...
            y_opts[:3] = (yy[np.newaxis,...] - lines + np.pi)%(2.*np.pi) - np.pi
            y_opts[3:6] = np.unwrap(y_opts[:3],discont=2.*np.pi-tol,axis=1)
        ...
        yslope = (np.copy(ph1-ph2)+np.pi)%(2.*np.pi)-np.pi

        if good_freqs is not None:
            yslope = yslope[good_freqs]
            fr = np.copy(self.freqs[good_freqs])

        yslope = np.unwrap(yslope,discont=2.*np.pi-tol,axis=0)
        ...
        c_ydists = a/1E6*C/(2.*np.pi*( np.cos(np.pi/2.-self.dec1+PHI)-
                                     np.cos(np.pi/2.-self.dec2+PHI) ) )
    ...
    def get_c_ydist_perfreq(self,ph1=None,ph2=None):
        ...
        dist_period = abs(C/(self.freqs*1E6*
                        ( np.cos(np.pi/2.-self.dec1+PHI)-
                        np.cos(np.pi/2.-self.dec2+PHI) ) ) )
        ...
        phdiff = ph1-ph2
        c_ydists = phdiff*C/(2.*np.pi*self.freqs[:,np.newaxis]*1E6*
                            ( np.cos(np.pi/2.-self.dec1+PHI)-
                            np.cos(np.pi/2.-self.dec2+PHI) ) )
        c_ydists = c_ydists % dist_period[:,np.newaxis]
        dist_opts = np.array([ c_ydists+base_up, c_ydists+base_ctr,
                                             c_ydists+base_down ])
        idxs = np.argmin( abs(dist_opts-c_ydists0[np.newaxis,np.newaxis,:]),
                                                                 axis=0 )
        c_ydists = np.array([ [ dist_opts[idxs[ii,jj],ii,jj]
                            for jj in range(self.Npr) ]
                            for ii in range(self.Nfr) ])
    ...
    def params_ft(self,tm,vis,dec,x0_shift=5.):
        ...
        ft_ord = np.concatenate((ft[...,Nt//2+Nt%2:],
                                 ft[...,:Nt//2+Nt%2]),axis=2)
        ft_ord = abs(ft_ord)
        ...
        mu0 = ( -2.*np.pi*freqs[:,np.newaxis]*1E6*distx_0[np.newaxis,:]*
                                np.sin(np.pi/2.-dec)/(3E8*SD) )
        ctr_idx = np.argmin(abs(fr_ord[np.newaxis,np.newaxis,:]-
                                        mu0[...,np.newaxis]),axis=2)
        A0 = np.array([ [ ft_ord[ ii, jj, ctr_idx[ii,jj] ]
                                    for jj in range(self.Npr) ]
                                    for ii in range(self.Nfr) ])
        # 1 deg => dt = 1*(pi/180)*(24*3600/2*pi) = 240s
        sigsqr0 = 1./(4.*np.pi**2*(240.*np.cos(dec))**2)
        p0 = np.array([ [ [ A0[ii,jj],mu0[ii,jj],sigsqr0 ]
                                    for jj in range(self.Npr) ]
                                    for ii in range(self.Nfr) ])
        # Perform fit:
        ...
        for ii in range(self.Nfr):
            for jj in range(self.Npr):
                try:
                    popt, pcov = curve_fit(gaus,fr_ord,
                                    ft_ord[ii,jj,:],p0[ii,jj])
    ...
    def get_xdist(self,ft_prms,dec):
        """ E-W
        """
        xdists = ( -ft_prms[...,1]*SD*C/(2.*np.pi*
                self.freqs[:,np.newaxis]*1E6*np.sin(np.pi/2.-dec)) )
    ...
    def data_quality(self):
        ...
        if self.pass_xd1 is None:
            if self.source2 is not None:
                self.xdist_test(self.c_xdists1,self.c_xdists2) # Assigns self.pass_xd1, self.pass_xd2
            else:
        ...
        if self.source2 is not None:
            ...
            self.good_prods = np.logical_and(np.logical_or(gpc1,gpc2),
                          np.logical_or(gpxd1,gpxd2)) # good prods
            self.good_freqs = np.logical_and(np.logical_or(gfc1,gfc2),
                          np.logical_or(gfxd1,gfxd2)) # good freqs

            # TODO: Delete these conservative estimates?
            self.good_prods_cons = np.logical_and(np.logical_and(gpc1,gpc2),
                               np.logical_and(gpxd1,gpxd2)) # Conservative good prods
            self.good_freqs_cons = np.logical_and(np.logical_and(gfc1,gfc2),
                               np.logical_and(gfxd1,gfxd2)) # Conservative good freqs
        else:
    ...
    def get_dists(self):
        ...
        self.c_xdists1 = (self.xdists1+self.c_ydists[np.newaxis,:]*np.sin(CR))/np.cos(CR)
        self.c_xdists2 = (self.xdists2+self.c_ydists[np.newaxis,:]*np.sin(CR))/np.cos(CR)
        self.ydists1 = ( self.xdists1+self.c_ydists[np.newaxis,:]*np.sin(CR)
                         )*np.tan(CR) + self.c_ydists[np.newaxis,:]*np.cos(CR)
        self.ydists2 = ( self.xdists2+self.c_ydists[np.newaxis,:]*np.sin(CR)
                         )*np.tan(CR) + self.c_ydists[np.newaxis,:]*np.cos(CR)
    ...
    def xdist_test(self,xds1,xds2=None,tol=2.):
        """
        """
        def get_centre(xdists,tol):
            ...
            cylseps = np.arange(-1,2)*22. if self.PATH else np.arange(-3,4)*22.
            devs = abs(xmeds[:,np.newaxis] - cylseps[np.newaxis,:])
            devmins = devs.min(axis=1)
            centres = np.array([xmeds[ii]                           # return median
    ...
    def good_prod_freq(self,pass_rst,tol_ch1=0.3,tol_ch2=0.7,tol_fr1=0.6,tol_fr2=0.7):
...
class ChanMonitor(object):
    ...
    def __init__(self,t1,t2=None,freq_sel=None,prod_sel=None,
                 bswp1=26,bswp2=90,bsep1=154,bsep2=218):
...
#        if freq_sel is not None:
    ...
    @classmethod
    def fromdate(cls,date,freq_sel=None,prod_sel=None,
                 bswp1=26,bswp2=90,bsep1=154,bsep2=218):
        """ Initialize class from date """
        t1 = ephemeris.datetime_to_unix(date)
        return cls(t1,freq_sel=freq_sel,prod_sel=prod_sel,
                   bswp1=bswp1,bswp2=bswp2,bsep1=bsep1,bsep2=bsep2)
    ...
    def get_src_cndts(self):
        ...
        grds = [ grd_dict[src.name]-2 if (
                                    (src.name in ['CygA','CasA']) and
                                    (not ntt[ii])
        ...
        src_cndts = [ src for grd,src in
                          sorted(zip(grds,srcs),
    ...
    def get_pol_prod_idx(self,pol_inpt_idx):
        ...
        for ii,prd in enumerate(self.prods):
            if ( ( prd[0] in pol_inpt_idx ) and
                 ( prd[1] in pol_inpt_idx ) ):
    ...
    def get_feedlocator(self,pol=1):
        ...
        if self.source2 is not None:
            fl = FeedLocator(self.dat1.vis[:,pol_prod_idx,:],
                         self.dat2.vis[:,pol_prod_idx,:],
                         self.tm1,self.tm2,
                         self.source1,self.source2,
                         self.freqs,prods,inputs,pstns,bsipts)
        else:
            fl = FeedLocator(self.dat1.vis[:,pol_prod_idx,:],
                         None,
                         self.tm1,None,
    ...
    def get_pos_pol(self,corr_inputs,pwds):
        ...
        for ii,inpt in enumerate(corr_inputs):
            if pwds[ii]:
                pstns[ii,0] = inpt.cyl*22.
                pstns[ii,1] = -1.*inpt.pos
                if inpt.pol=='S':
                    p1_idx.append(ii)
                else:
                    p2_idx.append(ii)
    ...
    def set_metadata(self,tms,input_map):
        ...
        self.pstns, self.p1_idx, self.p2_idx = self.get_pos_pol(self.corr_inputs,self.pwds)
    ...
    def get_prod_sel(self,data):
        ...
        for (ii,prod) in enumerate(data.prod):
            add_prod = False
            add_prod = add_prod or ( (prod[0]==self.bswp1 and prod[1] in echp1) or
                                     (prod[1]==self.bswp1 and prod[0] in echp1) )
            add_prod = add_prod or ( (prod[0]==self.bswp2 and prod[1] in echp2) or
                                     (prod[1]==self.bswp2 and prod[0] in echp2) )
            add_prod = add_prod or ( (prod[0]==self.bsep1 and prod[1] in wchp1) or
                                     (prod[1]==self.bsep1 and prod[0] in wchp1) )
            add_prod = add_prod or ( (prod[0]==self.bsep2 and prod[1] in wchp2) or
    ...
    def set_acq_list(self):
        ...
        fct = 3.
        tol1 = (np.arctan((mm-3.)*fct)+np.pi/2.)*10500./np.pi+1500.
        tol2 = (np.pi/2.-np.arctan((mm-11.)*fct))*10500./np.pi+1500.
        ttol = np.minimum(tol1,tol2)

        fct = 5.
        tol1 = (np.arctan((mm-4.)*fct)+np.pi/2.)*2100./np.pi+6000.
        tol2 = (np.pi/2.-np.arctan((mm-10.)*fct))*2100./np.pi+6000.
    ...
    def get_sunfree_srcs(self, srcs=None):
        ...
        for ii, src in enumerate(srcs):
            ...
            for acq in self.night_acq_list:
                night_transit = np.append(night_transit, ephemeris.transit_times(src, *acq[1]))
...
"""
            ...
            if self.good_freqs is None:
                ...
                self.good_freqs = np.logical_or(self.good_freqs,
                                                    self.flp2.good_freqs)
...
"""
    ...
    def results_summary(self):
        ...
        self.deemed_bad_but_good = self.input_map[ np.logical_and(
                                              np.logical_not(self.pwds),
                                              self.good_ipts ) ]
        self.bad_not_accounted = self.input_map[ np.logical_and( self.pwds,
                                              np.logical_not(self.good_ipts) )]
        if self.source2 is not None:
            # TODO: maybe use only x-position. Y is too erratic...
            self.pos_err = np.sum((self.postns-self.expostns)**2,axis=1)**0.5
            self.wrong_position = self.input_map[ self.pos_err>1. ]
