...
def waterfall(data, freq_sel=None, prod_sel=None, time_sel=None,
                part_sel=None, **kwargs):
    ...
    if (part_sel == 'complex'):
        msg = ('waterfall() does not take "complex" for "part_sel"'
                ' argument.')
    ...
    plt_data = _coerce_data_shape(data, freq_sel, prod_sel, time_sel,
                                    part_sel, axes=(1,))
    ...
    if med_filt != None:
        ...
        if med_filt[0] == 'new':
            # Apply median filter:
            plt_data, baseline = _med_filter(plt_data,med_filt[1],med_filt[2],med_filt[3])
            if len(med_filt)==5:
    ...
    if full_day != None:
        plt_data = _full_day_shape(plt_data,tmstp,date=full_day[0],
                                n_bins=full_day[1],axis=full_day[2],ax=ax)
    ...
    if show_plot == True:
        ...
        msg = ('Optional keyword argument "show_plot" should receive either'
                ' "True" or "False". Received "{0}". Ignoring argument.'
                .format(show_plot))
...
def spectra(data, freq_sel=None, prod_sel=None, time_sel=None,
            part_sel=None, **kwargs):
...
def time_ordered(data, freq_sel=None, prod_sel=None, time_sel=None,
                    part_sel=None, **kwargs):
...
def _coerce_data_shape(data, freq_sel=None, prod_sel=None, time_sel=None,
                        part_sel=None, axes=()):
    ...
    if data.ndim != 3:
        if data.ndim != 3 - len(axes):
            msg = ("Could no interpret input data axes. Got %dD data and need"
                   " coerse to %dD data") % (data.ndim, 3 - len(axes))
            raise ValueError(msg)
    ...
    if (part_sel == 'real' or part_sel == None):
        ...
        data = (data.real**2 + data.imag**2)**(.5)
    elif (part_sel == 'phase'):
        data = np.arctan(data.imag/data.real)
    elif (part_sel == 'complex'):
        pass
    else:
        msg = ('Optional keyword argument "part_sel" has to receive'
    ...
    if axis == 'solar':
        ...
        for ii in range(len(tmstp)):
            ...
            if in_range:
                ...
                in_start_range = (tmstp[ii]>start_range[0]) and (tmstp[ii]<start_range[1])
        ...
        if ax != None:
            tck_stp = n_bins/6.
            ticks = np.array([int(tck_stp),int(2*tck_stp),int(3*tck_stp),int(4*tck_stp),int(5*tck_stp)])
            ax.set_xticks(ticks)
...
def _med_filter(data,n_bins=200,i_bin=0,filt_window=37):
    ...
    sigma = np.median(std_arr)/(sliced2darray.shape[1])**(.5)
    ...
    interpolat_arr_baseline = np.interp(freq, freq[non_rfi_mask],
                                                   mean_arr[non_rfi_mask])
    # Median filter a second time:
    small_window = int(filt_window//3)
    # Has to be odd:
    if small_window % 2 == 0:
        small_window = small_window + 1
    medfilt_baseline = np.reshape(sig.medfilt(interpolat_arr_baseline, small_window), 
