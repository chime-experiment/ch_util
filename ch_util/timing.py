...
class TimingCorrection(andata.BaseData):
    ...
    def _interpret_and_read(cls, acq_files, start, stop, datasets, out_group):
        ...
        for tt, obj in enumerate(objs):
            for nn in range(dphi.shape[1]):
                if np.any(flag[:, nn, tt]):
                    ...
                    param = fit_poly_to_phase(freq, np.exp(1.0J * dphi[:, nn, tt]),
                                              err[:, nn, tt], nparam=2)[0]
        ...
        flag = (weight > 0.0) & (weight_ref > 0.0)
        weight = tools.invert_no_zero(tools.invert_no_zero(weight) +
                                      tools.invert_no_zero(weight_ref))
        ...
        data = tod.concatenate(objs, out_group=out_group, start=start,
    ...
    def set_global_reference_time(self, tref, interpolate=False, **kwargs):
        ...
        if (tref < self.time[0]) or (tref > self.time[-1]):
            InputError("Timing correction not available for time %s." %
    ...
    def set_reference_time(self, tref, tstart, tend=None,
                           tinit=None, tau_init=None, alpha_init=None,
        ...
        if tau_init is None:
            ...
            if tau_init.size == tau_ref.shape[0]:
                ...
                InputError("Initial tau has size %d, but there are %d noise sources." %
        ...
        if alpha_init is None:
            ...
            if alpha_init.size == alpha_ref.shape[0]:
                ...
                InputError("Initial alpha has size %d, but there are %d noise sources." %
        ...
        alpha_ref_full = coeff * alpha_ref[:, istart] + (1.0 - coeff) * alpha_ref[:, iend]
    ...
    def get_tau(self, timestamp, ignore_amp=False, interp='linear', extrap_limit=None):
        ...
        if ignore_amp or (self.amp_to_delay is None) or not self.has_amplitude:
            ...
            tau = self._interp_time(self.tau[:], timestamp,
                                    interp=interp, extrap_limit=extrap_limit)

        else:

            logger.info("Correcting delay template using amplitude template "
            ...
            if iref is None:
                raise RuntimeError("Could not determine which input the delay template "
                                   "is referenced with respect to.")

            # Subtract the referenced, scaled alpha template from the delay template
            tau_corrected = self.tau[:] - self.amp_to_delay * (self.alpha[:] -
                                                               self.alpha[iref, np.newaxis, :])

            # Interpolate to the requested times
            tau = self._interp_time(tau_corrected, timestamp,
                                    interp=interp, extrap_limit=extrap_limit)
    ...
    def get_alpha(self, timestamp, interp='linear', extrap_limit=None):
        ...
        """
        alpha = self._interp_time(self.alpha[:], timestamp,
                                  interp=interp, extrap_limit=extrap_limit)
        return alpha
    ...
    def _interp_time(self, data, timestamp, interp='linear', extrap_limit=None):
        ...
        interpolator = interp1d(self.time, data, axis=-1,
                                kind=interp, fill_value='extrapolate')
        data_interp = interpolator(timestamp)

        return data_interp

    def get_stacked_tau(self, timestamp, inputs, prod, stack_index, input_flags=None, **kwargs):
        ...
        """
        return self._stack(self.get_tau, timestamp, inputs, prod, stack_index,
                           input_flags=input_flags, **kwargs)

    def get_stacked_alpha(self, timestamp, inputs, prod, stack_index, input_flags=None, **kwargs):
        ...
        """
        return self._stack(self.get_alpha, timestamp, inputs, prod, stack_index,
                           input_flags=input_flags, **kwargs)

    def _stack(self, func, timestamp, inputs, prod, stack_index, input_flags=None, **kwargs):
        ...
        dt = [(native_str('stack'), '<u4'), (native_str('input_a'), '<u2'),
        ...
        if (input_flags is None) or not np.any(input_flags) or np.all(input_flags):
            ...
            weight = (weight.astype(np.float32) *
                      tools.invert_no_zero(weight_norm[unique_prod['stack']].astype(np.float32)))

            for ii, up in enumerate(unique_prod):
                stacked_data[up['stack'], :] += weight[ii] * (data[up['input_a'], :] -
                                                              data[up['input_b'], :])
        else:

            # Find boundaries into the sorted products that separate stacks.
            boundary = np.concatenate((np.atleast_1d(0),
            ...
            for ss in range(nstack):
                ...
                for ii, up in enumerate(unique_prod):
                    ...
                    ww = np.sum(input_flags[prodo['input_a'][this_group], :] *
                                input_flags[prodo['input_b'][this_group], :], axis=0)

                    weight_norm[up['stack'], :] += ww
                    stacked_data[up['stack'], :] += ww * (data[up['input_a'], :] -
                                                          data[up['input_b'], :])
    ...
    def get_timing_correction(self, freq, timestamp, **kwargs):
        ...
        return np.exp(-1.0J * FREQ_TO_OMEGA * freq[:, np.newaxis, np.newaxis] *
                      tau[np.newaxis, :, :])
    ...
    def apply_timing_correction(self, timestream, copy=False, **kwargs):
        ...
        if isinstance(timestream, np.ndarray):
            ...
            reverse_stack = kwargs.pop('reverse_stack',
                                       timestream.reverse_map['stack']['stack'][:])

        input_flags = kwargs.pop('input_flags', None)

        # Determine if the visibilities have been stacked
        is_stack = ((stack is not None) and (reverse_stack is not None) and
                    (stack.size < prod.size))
        ...
        if is_stack:
            ...
            tau = self.get_stacked_tau(timestamp, inputs, prod, reverse_stack,
                                       input_flags=input_flags, **kwargs)
    ...
    def summary(self):
        ...
        sig2_tau = np.sqrt(np.sum(np.diff(self.tau[:], axis=-1)**2, axis=-1) /
        ...
        for ii, inp in enumerate(self.noise_source):
            summary.append(fmt % (inp['correlator_input'],
                                  self.static_phi_fit[0, ii],
                                  self.static_phi_fit[1, ii] * 1e-3,
                                  sig2_tau[ii], sig_tau[ii]))
...
class TimingData(andata.CorrData, TimingCorrection):
    ...
    def from_acq_h5(cls, acq_files, only_correction=False, **kwargs):
        ...
        corr_data = super(TimingData, cls).from_acq_h5(acq_files,
        ...
        data = TimingCorrection(**dist_kwargs) if only_correction else TimingData(**dist_kwargs)
        ...
        if only_correction:
            ...
            for name, old_dset in corr_data.datasets.items():
                new_dset = data.create_dataset(name, data=old_dset[:],
                                               distributed=old_dset.distributed)
            ...
            for name, old_dset in corr_data.flags.items():
                new_dset = data.create_flag(name, data=old_dset[:],
                                            distributed=old_dset.distributed)
                memh5.copyattrs(old_dset.attrs, new_dset.attrs)

        # Construct delay template
        res = construct_delay_template(corr_data, **cdt_kwargs)
        tau, static_phi, w_static_phi, static_phi_fit, alpha, static_amp, w_static_amp = res
    ...
    def summary(self):
        ...
        vis = self.apply_timing_correction(self.vis[:], copy=True,
                                           freq=self.freq, time=self.time,
                                           prod=self.prod, input=self.input)

        phi_before = np.angle(self.vis[:])
        phi_after = np.angle(vis)

        phi_before = _correct_phase_wrap(phi_before -
                                         np.median(phi_before, axis=-1)[..., np.newaxis])
        phi_after = _correct_phase_wrap(phi_after -
                                        np.median(phi_after, axis=-1)[...,  np.newaxis])

        sig_before = np.median(np.abs(phi_before -
                                      np.median(phi_before, axis=-1)[..., np.newaxis]), axis=-1)
        sig_after = np.median(np.abs(phi_after -
                                     np.median(phi_after, axis=-1)[..., np.newaxis]), axis=-1)
        ...
        for ii, pp in enumerate(self.prod):
            if pp[0] != pp[1]:
                summary.append(fmt % ((self.input[pp[0]]['correlator_input'],
...
def load_timing_correction(files, start=None, stop=None,
                           window=43200.0, instrument='chime', **kwargs):
    ...
    if not acq_inst.startswith(instrument) or (acq_type != 'corr'):
        raise RuntimeError("This function is only able to parse corr type files "
                           "from the specified instrument (currently %s)." % instrument)

    # Search for all timing acquisitions on this node
    tdirs = sorted(glob.glob(os.path.join(node, '_'.join(['*', instrument + 'timing', acq_type]))))
    if not tdirs:
    ...
    tacq_start = np.array([ephemeris.timestr_to_datetime(os.path.basename(tt)) for tt in tdirs])
...
def construct_delay_template(data, check_amp=True, check_sig=True, nsigma=5.0, threshold=0.50,
                             nparam=2, min_freq=420.0, max_freq=780.0, max_iter_weight=2,
                             static_phi=None, weight_static_phi=None, static_phi_fit=None,
                             static_amp=None, weight_static_amp=None):
    ...
    if weight_static_phi is not None:
        weight_static_phi, wsphi_shp, wsphi_ind = _resolve_distributed(weight_static_phi, axis=1)
        assert wsphi_shp == (nfreq, nsource)

    if static_phi_fit is not None:
        static_phi_fit, sphifit_shp, sphifit_ind = _resolve_distributed(static_phi_fit, axis=1)
    ...
    if weight_static_amp is not None:
        weight_static_amp, wsamp_shp, wsamp_ind = _resolve_distributed(weight_static_amp, axis=1)
    ...
    if check_amp or check_sig:
        ...
        if check_amp:
            ...
            good_amp *= ((np.sum(good_amp, axis=-1) /
                          float(good_amp.shape[-1])) > threshold)[:, :, np.newaxis]
        ...
        if check_sig and (weight is not None):
            ...
            good_wsig *= ((np.sum(good_wsig, axis=-1) /
                           float(good_wsig.shape[-1])) > threshold)[:, :, np.newaxis]
    ...
    if nsource > 2:
        ...
        wwp = np.repeat(flg[:, 0, np.newaxis, :] *
                        flg[:, 1, np.newaxis, :], 2, axis=1)
    ...
    tau = np.sum(wwp * omega * phi, axis=0) * tools.invert_no_zero(np.sum(wwp * omega**2, axis=0))

    # Estimate variance of each frequency from residuals
    inv_var_weight = wwp
    for iter_weight in range(max_iter_weight):

        residual_var = (np.sum(wwp * (phi - omega * tau[np.newaxis, :, :])**2, axis=-1) *
    ...
    if static_phi_fit is None:
        ...
        for nn in range(nsource):
            if np.any(err_static_phi[:, nn] > 0.0):
                static_phi_fit[:, nn] = fit_poly_to_phase(freq, np.exp(1.0J * static_phi[:, nn]),
                                                          err_static_phi[:, nn],
                                                          nparam=nparam)[0]
    else:
    ...
    alpha = np.sum(wwa * asc * amp, axis=0) * tools.invert_no_zero(np.sum(wwa * asc**2, axis=0))

    # Estimate variance of each frequency from residuals
    inv_var_weight = wwa
    for iter_weight in range(max_iter_weight):

        residual_var = (np.sum(wwa * (amp - asc * alpha[np.newaxis, :, :])**2, axis=-1) *
    ...
    # Return results
    return (tau, static_phi, weight_static_phi, static_phi_fit,
            alpha, static_amp, weight_static_amp)
...
def map_input_to_noise_source(inputs, noise_sources):
    ...
    imap = [np.argmax([count_startswith(inp, src) for src in source_names]) for inp in input_names]
...
def fit_poly_to_phase(freq, resp, resp_error, nparam=2):
    ...
    if flg.size < (nparam + 1):
        msg = "Number of data points must be greater than number of parameters (%d)." % nparam
    ...
    p0[1] = np.median(np.diff(np.angle(y_complex)) / (FREQ_TO_OMEGA * np.diff(freq[flg])))

    # Try nonlinear least squares fit
    try:
        popt, pcov = curve_fit(_func_poly_phase, x, y, p0=p0, sigma=err, absolute_sigma=False)
...
def _resolve_distributed(arr, axis=1):
    if isinstance(arr, mpiarray.MPIArray):
        ...
        ilocal = list(range(arr.local_offset[axis],
                            arr.local_offset[axis] + arr.local_shape[axis]))
    else:
